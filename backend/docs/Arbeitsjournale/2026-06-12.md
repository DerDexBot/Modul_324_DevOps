...
### 8. Bugfix: Runner-Konfiguration und Maven-Installation

#### 8.1 REPO_URL Platzhalter

**Problem:** Der Runner-Container startete und crashte sofort in einer Endlosschleife (`restart: unless-stopped`). Logs zeigten:
```
curl: (22) The requested URL returned error: 404
Invalid configuration provided for token. Terminating unattended configuration.
```

**Ursache:** In `runner/.env` stand noch der Platzhalter `REPO_URL=https://github.com/DerDexBot/REPO_NAME` aus dem Template – der `REPO_NAME` wurde nicht ersetzt.

**Fix:** `REPO_URL=https://github.com/DerDexBot/Modul_324_DevOps` eingetragen. Runner registrierte sich danach sofort erfolgreich (`√ Runner successfully added` → `Listening for Jobs`).

---

#### 8.2 Maven nicht auf Self-Hosted Runner installiert

**Problem:** Backend Tests schlugen in ~13 Sekunden fehl (viel zu schnell für Maven-Tests). GitHub Actions Log zeigte:
```
mvn: command not found
##[error]Process completed with exit code 127.
```

**Ursache:** `myoung34/github-runner` ist ein minimales Ubuntu-Image – Maven ist **nicht** vorinstalliert. Auf GitHub-hosted `ubuntu-latest` Runnern ist Maven immer dabei, auf Self-Hosted Runnern nicht. `actions/setup-java` installiert nur den JDK, nicht Maven.

**Fix:** Expliziter Install-Schritt in `ci.yml` und `cd.yml` vor dem Build:
```yaml
- name: Install Maven
...
**Lernerkenntnisse:**
- GitHub-hosted Runner haben viele Tools vorinstalliert (Maven, Node, Docker CLI etc.) – Self-Hosted Runner sind minimal und müssen explizit eingerichtet werden
- Exit code 127 = "command not found" – ein klares Signal dass ein Tool fehlt
- `restart: unless-stopped` ist nützlich für Produktionsbetrieb, kann aber eine Crash-Loop verstecken – Logs immer mit `docker logs` oder `docker compose logs` prüfen
- `docker compose logs -f` oder `docker logs -f <container>` zeigt den Live-Status des Runners
...
| CD: JAR-Artefakt Upload | Aktualisiert ✓ |
| CI Frontend: Vite-Build + dist/-Artefakt | Aktualisiert ✓ |
| Runner: REPO_URL Platzhalter behoben | Gefixt ✓ |
| Runner: Maven-Installation ergänzt | Gefixt ✓ |
...
flowchart TD
    ...
    subgraph RUNNER["Self-Hosted Runner · Docker · runner/docker-compose.yml"]
        subgraph CIB_block["CI Backend · ci.yml"]
            CIB["JDK 21 · Maven install\nmvn verify – 31 Tests · CodeQL"]
        end
        subgraph CIF_block["CI Frontend · ci-frontend.yml"]
            CIF["Node.js 24 · Jest – 9 Tests\nESLint · Vite Build"]
        end
        subgraph CD_block["CD · cd.yml · nur wenn CI Backend OK"]
            CD["Maven install · JAR bauen\nDocker Image · Push ghcr.io"]
        end
    end
    ...
    CIF --> FE_ART([todo-frontend-n.zip\nArtifact · 30 Tage])
...
- Maven-Installation cachen um Build-Zeit zu reduzieren
